"""Publishing, campaigns, scheduling and publish-attempt lifecycle.

Attempts are a state machine, and the transitions a caller may drive are enumerated as frozensets
(``RESUMABLE_PUBLISH_STATES``, ``RESCHEDULABLE_PUBLISH_STATES``) rather than checked inline, so an
attempt cannot be resumed or rescheduled out of a state that has already been settled.
"""
...
from api.models import CampaignModel, PublishClipRequest, RescheduleModel
from config import settings
from publishers import best_times
from publishers.base import PublishState

#: Every route below carries the ``publishing`` tag, supplied here rather
...
def publisher_statuses() -> dict:
    return {"platforms": deps.get_publish_manager().statuses()}


@router.get("/api/campaigns")
def list_campaigns() -> dict:
    return {"campaigns": [c.to_dict() for c in deps.get_history().campaigns()]}
...
def save_campaign(req: CampaignModel) -> dict:
    ...
    return deps.get_history().save_campaign(req.name.strip(), req.routes, req.id).to_dict()


@router.post("/api/jobs/{job_id}/clips/{clip_id}/publish")
def publish_clip(job_id: str, clip_id: str, req: PublishClipRequest) -> dict:
    ...
    ids=deps.get_publish_manager().submit(job_id=job_id,clip=clip,video_path=path,
    ...
    return {"attempt_ids":ids,"attempts":[deps.get_history().get_attempt(i) for i in ids]}


@router.get("/api/history")
def history(limit: int=200, platform: str="") -> dict:
    return deps.get_history().history(max(1,min(limit,500)),platform)


@router.get("/api/publish-attempts/{attempt_id}")
def publish_attempt(attempt_id: str) -> dict:
    item=deps.get_history().get_attempt(attempt_id)
    if not item:
...
def _resume_attempt(attempt_id: str, *, force_direct: bool) -> dict:
    ...
    manager = deps.get_publish_manager()
...
@router.post("/api/publish-attempts/{attempt_id}/approve")
...
def schedule_window(start: Optional[float] = None, end: Optional[float] = None) -> dict:
    ...
    return {
        "start": begin,
        "end": finish,
        "attempts": deps.get_history().scheduled_between(begin, finish),
    }


@router.get("/api/schedule/suggestions")
def schedule_suggestions(platform: str = "", days: int = 7, per_day: int = 2) -> dict:
    ...
    taken = [
        ...
        for a in deps.get_history().scheduled_between(now, now + horizon * 86400)
...
@router.patch("/api/publish-attempts/{attempt_id}/schedule")
...
@router.post("/api/publish-attempts/{attempt_id}/cancel")
...
@router.post("/api/publishers/{platform}/refresh")
...
@router.post("/api/publish-attempts/{attempt_id}/retry")
