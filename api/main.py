...
This module builds the app and declares **no routes** beyond the SPA fallback page. Every endpoint
lives in ``api/routers/``, one module per OpenAPI tag:

    system      /healthz, /api/info
    input       /api/preview
    jobs        upload, URL ingest, batches, status, cancel, resume
    metadata    per-clip editing, review state, caption preview, re-render, regeneration
    publishing  publishers, campaigns, scheduling, publish-attempt lifecycle
    storage     disk usage, retention settings, cleanup, source deletion
    profiles    saved and built-in option profiles
    updates     /api/updates
    watch       the watch folder
    clips       downloads (metadata zip, bare video)

It deliberately does not restate the endpoint list. It used to, and the list had drifted: an
inventory maintained by hand next to the code it describes is a document that gets out of date
without anything failing. ``GET /openapi.json`` is generated from the routes themselves and is
always correct.

What has to stay here, and why
------------------------------
* ``app`` itself, with the app-level ``dependencies=[Depends(require_api_token)]``. Registered once
  rather than on 46 decorators, which is what made this split safe -- the auth survived moving
  every route between modules.
* The middleware stack, whose *order* is behaviour (see the comments on each).
* The startup security check and the lifespan that runs it.
* The static mounts, which must come **after** every router: the frontend is mounted at ``/`` with
  ``html=True`` and matches every path.
* ``fallback_index_html``, the page served when the frontend has not been built. It reports the
  live state of the deployment, so it reaches into ``api.capabilities`` and ``api.deps`` the same
  way a route would.
...
from api.capabilities import _engines_info
...
# Routes
# ---------------------------------------------------------------------------
# One router per OpenAPI tag; this module declares no routes of its own beyond the SPA fallback
# below. The tag *is* the module boundary, and
# `tests/test_api_route_table.py::test_every_api_route_has_exactly_one_tag` keeps it that way, so
# a route added later cannot default into living here.
#
# Registered *before* the static mounts, and that ordering is load-bearing rather than tidy: the
# frontend is mounted at "/" with `html=True`, which matches every path. A router included after it
# would have all of its routes answered with `index.html` and a 200 -- no error, no failing test,
# and nothing visibly wrong until someone opened the app.
# `test_the_spa_catch_all_mount_is_last` is what holds this.
for _router in (
    system.router,
    input.router,
    jobs.router,
...
def fallback_index_html() -> str:
    ...
    try:
        jobs = deps.get_manager().store.all()
