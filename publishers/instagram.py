...
class InstagramPublisher(BasePublisher):
    ...
    def publish(self,request):
        ...
        try:
            ver=settings.instagram_api_version; ig=st.account_id; token=settings.instagram_access_token
            create=self.client.post(f"https://graph.facebook.com/{ver}/{ig}/media",json={"media_type":"REELS","upload_type":"resumable","caption":request.caption,"access_token":token})
            create.raise_for_status(); cid=create.json()["id"]; size=request.video_path.stat().st_size
            with request.video_path.open("rb") as f:
                up=self.client.post(f"https://rupload.facebook.com/ig-api-upload/{ver}/{cid}",content=f,
                  headers={"Authorization":f"OAuth {token}","offset":"0","file_size":str(size)})
            up.raise_for_status()
            if request.mode=="review":
                return PublishResult(True,PublishState.DRAFT,self.name,external_id=cid,message="Reel container uploaded; publish within 24 hours")
            for _ in range(5):
                status=self.client.get(f"https://graph.facebook.com/{ver}/{cid}",params={"fields":"status_code","access_token":token})
                status.raise_for_status(); code=status.json().get("status_code")
                if code in ("FINISHED","PUBLISHED"): break
                if code in ("ERROR","EXPIRED"): raise RuntimeError(f"Instagram container {code.lower()}")
                self.sleep(1)
            pub=self.client.post(f"https://graph.facebook.com/{ver}/{ig}/media_publish",json={"creation_id":cid,"access_token":token})
            pub.raise_for_status(); media_id=pub.json()["id"]
            return PublishResult(True,PublishState.PUBLISHED,self.name,external_id=media_id,message="Reel published",raw=pub.json())
