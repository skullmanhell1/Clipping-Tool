...
class TikTokPublisher(BasePublisher):
    ...
    def publish(self,request):
        ...
        try:
            ...
            endpoint=("https://open.tiktokapis.com/v2/post/publish/video/init/" if direct else
            ...
            if direct:
                body["post_info"]={"title":request.caption[:2200],"privacy_level":"PUBLIC_TO_EVERYONE",
                   "disable_duet":False,"disable_comment":False,"disable_stitch":False}
            r=self.client.post(endpoint,json=body,headers={"Authorization":f"Bearer {settings.tiktok_access_token}","Content-Type":"application/json; charset=UTF-8"})
            r.raise_for_status(); data=r.json(); err=data.get("error",{})
            if err.get("code") not in (None,"ok"):
                raise RuntimeError(err.get("message") or err.get("code"))
            target=data["data"]["upload_url"]; pub_id=data["data"]["publish_id"]
            with request.video_path.open("rb") as f:
                up=self.client.put(target,content=f,headers={"Content-Type":"video/mp4","Content-Length":str(size),"Content-Range":f"bytes 0-{size-1}/{size}"})
            up.raise_for_status()
            state=PublishState.PUBLISHED if direct else PublishState.DRAFT
            msg="Direct post submitted for moderation" if direct else "Draft uploaded; finish in TikTok inbox"
