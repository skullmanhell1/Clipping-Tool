      ...
      {showEffects && (
        <div className="mt-4 space-y-5 rounded-xl border border-slate-800 bg-slate-950/40 p-4">
          ...
          <fieldset disabled={!kineticAvailable} className={kineticAvailable ? "" : "opacity-60"}>
            {/* A real <legend>, not a styled <div>: a fieldset without one has no accessible
                name, so a screen reader announces the grouped controls with no indication of
                what groups them -- and "disabled" with no group label gives no reason why. */}
