...
def _capture(engine: Any, ctx: Engine_Context) -> tuple[bool, Any]:
    ...
    except Exception as exc:  # Reqs 8.1, 8.4: isolate, never propagate
        return False, exc
...
def filler_seam_notes(keeps: Any) -> tuple[str, ...]:
    ...
    try:
        items = list(keeps)
    except Exception:  # a hostile plan must not break publication
        return ()
    if len(items) < 2:
        return ()
...
class Engine_Host:
    ...
    def finish_clip(self, clip_id: str) -> list[str]:
        ...
        *job-level* scratch space only (Reqs 17.2, 17.3, 17.6) — which is where
        ...
        for engine_id, artifact, results, index in self._durables.pop(key, []):
            ...
            except Exception as exc:  # Req 18.6: degrade, never fail
                self._warn(
    ...
    def _invoke(
        ...
        try:
            ctx = ctx_factory()
        except Exception as exc:  # Reqs 8.1, 8.5
    ...
    def _execute(
        ...
        * the **deadline check** — after a normal return, the injected clock is
        ...
        try:
            ...
            except Exception as exc:  # Reqs 8.1, 8.4, 8.5
    ...
    def _all_engines(self) -> list[AV_Engine]:
        ...
        except Exception as exc:  # a hostile registry must not break gating
    ...
    def _is_enabled(self, engine: Any) -> bool:
        ...
        except Exception as exc:  # an unreadable flag reads as disabled
