...
def plan_kinetic(
    ...
    *,
    ...
    for (cue_start, cue_end), (_s, _e, cue_words, lines) in zip(filled_bounds, drafts, strict=True):
    ...
    degraded = False
    detail = (
        f"{len(cues)} cues, {word_count} words, " f"style={options.style}, reveal={options.reveal}"
    )
...
def _style_span(
    ...
    if style == "pop":
        return f"{{\\fscx60\\fscy60\\t({rel},{rel + 120}," f"\\fscx100\\fscy100)}}{escaped}"
    ...
    if style == "highlight_sweep":
        return f"{{\\c{highlight}&\\t({rel},{rel + duration_ms}," f"\\c{primary}&)}}{escaped}"
