...
def zoom_filter(
    ...
    *,
    ...
    return (
        f"zoompan=z='{expr}':d=1:fps={fps:g}:s={width}x{height}:x='{x_expr}':y='ih/2-(ih/zoom/2)'"
    )
...
def video_fade_filter(duration: float, fade_dur: float = 0.4) -> str | None:
    ...
    return f"fade=t=in:st=0:d={fade_dur:.3f},fade=t=out:st={out_start:.3f}:d={fade_dur:.3f}"
