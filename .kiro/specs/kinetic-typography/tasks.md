...
- [ ] 1. Prerequisite gate on the AV engines foundation
  - [ ] 1.1 Verify the foundation modules and contracts this engine binds to exist
    ...
    - Confirm `worker/effects/compositor.py` `render_clip` already accepts the foundation's `compose_contributions` keyword-only parameter (this plan only adds a branch inside it, in task 12.1) — record the exact parameter name if it differs, and use that name throughout epic 12 instead of introducing a second one.
...
- [ ] 2. Engine-specific generators (`tests/strategies.py`)
  - [ ] 2.1 Add the six kinetic generators to the existing shared generator module
