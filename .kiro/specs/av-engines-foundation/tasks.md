...
- [x] 15. Clip_Metadata seam on `Engine_Context`
  - **Epic 15 complete — the channel exists, the Pipeline fills it, and the parity gate is untouched.** `Engine_Context.clip_metadata` (last field, after `deps`) + keyword-only `run_stage(clip_metadata=...)` snapshotted once per stage run and copied into **every** engine's context (15.1); the 13.3 contract pin updated for the new field and a `run_stage` signature pin newly created (15.2); four unit tests for reach, inert default, verbatim pass-through and the read-only boundary (15.3); one Pipeline-driven integration test that fails when the `pipeline.py` line is removed (15.4); and the all-off gate re-run unmodified (15.5). `deps` was never touched, and no existing assertion was weakened anywhere. Full suite: **321 passed / 53 skipped / 0 failed**, twice, versus the 317/52 baseline — +4 passed are the 15.3 unit tests and +1 skipped is the 15.4 ffmpeg-gated integration test (skipped only because this sandbox has no ffmpeg; it passes when a real ffmpeg is on `PATH`).
  - [x] 15.1 Carry `clip_metadata` on `Engine_Context` and accept it in `run_stage`
    ...
    - **Completed 15.1 — additive, inert by default, `deps` untouched; full suite unchanged at 317 passed / 52 skipped.** `clip_metadata: Mapping[str, Any] = field(default_factory=dict)` is the **last** field of `Engine_Context`, after `deps`, and `__post_init__` normalises it exactly like `deps` — a non-`Mapping` value becomes the documented empty mapping, and a real mapping's keys and values are left completely untouched (no filtering, coercion, renaming or defaulting). Three shipped decisions worth carrying forward: (a) `run_stage`'s keyword-only `clip_metadata: Mapping[str, Any] | None = None` is **snapshotted once per stage run** in `Engine_Host._run` (`dict(clip_metadata) if isinstance(..., Mapping) else {}`) and threaded through the per-engine `ctx_factory` closure, so **every** engine of the stage receives it, not just the first — the `first_input_index` offsets already forced that closure to be per-engine, so no new plumbing was needed. (b) `_build_context` takes its own copy (`dict(clip_metadata)`) per context, so an engine can reach neither the caller's mapping nor another engine's context through it — a shallow copy, so nested mutable values are still shared by reference, which is the right cost for a read-only planning channel and is the boundary 15.3 should assert. (c) The parameter was added to the shared `_run` body rather than only to `run_stage`, so `run_source` keeps passing nothing and SOURCE-stage contexts get the empty default (a source-stage run has no per-clip upstream values by definition). `deps` is byte-for-byte as it was.
    - _Requirements: 15.8_

  - [x] 15.2 Update the Task 13.3 contract-surface pin for the new field and parameter
    ...
    - **Completed 15.2 — one field appended to the field-order pin, and the `run_stage` parameter pin newly *created*; no other test assertion touched.** `"clip_metadata"` was appended (last, after `"deps"`) to `_PINNED_FIELDS["Engine_Context"]` in `tests/test_engines_base.py`, with a comment recording *why* a designed addition always lands at the end. **One judgement call, carried forward:** the task says to "add `clip_metadata` to the pinned `Engine_Host.run_stage` parameter list", but **no such pin existed** — task 13.3's `test_engine_method_surface_is_pinned` pinned `base` / `registry` / `capabilities` / `timebase` / `artifacts` and never imported `worker.engines.host` at all (the earlier `first_input_index` work pinned the *field*, not the host signature). Rather than treat the sub-task as a no-op, the pin was **created** in that same test: `inspect.signature(Engine_Host.run_stage)` is asserted to be exactly `[self, stage, clip_id, source, clip_path, clip_start, clip_end, duration, words, clip_metadata]`, plus `clip_metadata` being `KEYWORD_ONLY` with default `None` — so the host's stage entry point is now covered by the same visible-contract-change discipline as the records. The host import is function-local, matching the file's existing pattern and keeping `test_every_engine_module_imports_without_heavy_dependencies` unaffected. Verified by mutation on both halves: moving `clip_metadata` ahead of `deps` on `Engine_Context`, and dropping the keyword from `run_stage`, each fail the pin.
    - _Requirements: 15.8, 23.6_

  - [x]* 15.3 Unit tests: Clip_Metadata reaches every engine unchanged → `tests/test_engine_host.py`
    ...
    - **Completed 15.3 — four unit tests appended to `tests/test_engine_host.py`; no existing assertion touched.** `test_clip_metadata_reaches_every_engine_of_the_stage_run` registers **three** COMPOSE engines and asserts each one's recorded `ctx.clip_metadata` equals the supplied mapping, so the "merged into the first context only" regression is caught — verified by mutation (clearing the per-stage-run snapshot after the first invocation fails this test and the read-only test, and leaves the other two green). `test_omitted_clip_metadata_is_empty_and_leaves_the_context_otherwise_unchanged` runs the same stage three times on one host — keyword omitted entirely, mapping supplied, explicit `None` — and compares **every** `dataclasses.fields` entry except `clip_metadata`; the injected `FakeClock` never self-advances, so `deadline` is identical across the runs and the field-by-field comparison is meaningful rather than vacuous. Omitted and explicit `None` both yield `{}`. `test_clip_metadata_unknown_keys_pass_through_untouched` asserts value **identity** (`seen[key] is value`) over a hostile payload — a tuple that must not become a list, a numeric string that must not become an int, a `None` that must not be defaulted away, a nested structure, and an empty-string key — so any filtering, coercion, renaming or defaulting fails. `test_clip_metadata_is_read_only_from_the_engine_point_of_view` covers three boundaries: rebinding `ctx.clip_metadata` raises `FrozenInstanceError`; caller mapping, engine A's and engine B's are three distinct objects; and writing into one engine's copy leaves both the caller's mapping and the sibling engine's context unchanged. **Judgement call:** that last mutation is performed on the recorded context from the test body rather than from a bespoke mutating engine double — the module's stated rule is that doubles live in `tests/fakes.py` and are never redefined locally, and the recorded object *is* the one the engine held, so the boundary asserted is identical. The shallow-copy boundary recorded in 15.1 (nested mutable values are still shared by reference) is stated in the docstring, and the assertions deliberately stay at the top level.
    - _Requirements: 15.8_

  - [x] 15.4 Integration test: the Pipeline actually supplies Clip_Metadata at the COMPOSE hook → `tests/test_pipeline_effects.py`
    ...
    - **Completed 15.4 — the Pipeline now supplies Clip_Metadata at the COMPOSE hook, and the test provably fails without it.** One additive argument in `worker/pipeline.py`, on the COMPOSE `host.run_stage(...)` call only (the other four hooks are byte-for-byte unchanged): `clip_metadata={"hook_text": md.hook_text, "clip_size": fu.ASPECT_PRESETS.get(options.aspect, fu.ASPECT_PRESETS["9:16"])}`. `md.hook_text` is already in scope — the very next statement hands it to `compositor.render_clip` — and `ProcessingOptions.aspect` is the real field (there is no `aspect_ratio`), so the presets lookup needs no translation layer; `.get(..., 9:16)` keeps an out-of-range aspect from raising inside the hook. `test_pipeline_supplies_clip_metadata_to_compose_stage_engines` in `tests/test_pipeline_effects.py` drives the **real** `run_pipeline` (`make_video` + `requires_ffmpeg`, isolated registry/report/storage/clock via the existing `_inject_isolated_host`) with a recording `FakeEngine` and supplies **no** `clip_metadata` of its own. Two deliberate choices make it genuinely discriminating: (a) `options.metadata` is **ON** with a `MockLLMClient` injected through `run_pipeline(llm_client=...)`, so `hook_text` travels the Pipeline's own `meta_mod.generate_metadata` path — stubbing `generate_metadata` would have re-introduced exactly the by-hand-mapping blind spot this task exists to close, and the metadata-off fallback produces an empty `hook_text`; and (b) `aspect="4:5"`, **not** the `9:16` default, with an explicit `!= ASPECT_PRESETS["9:16"]` assertion, so a `clip_size` hardcoded to the fallback preset fails. It also cross-checks `seen["hook_text"] == clip.hook_text` and `probe_size(out) == ASPECT_PRESETS["4:5"]`. **Verified by reverting:** with the `clip_metadata=` argument removed from `pipeline.py` the test fails on `assert seen` ("the Pipeline supplied no Clip_Metadata at the COMPOSE hook", `assert {}`); the argument was then restored. **Environment note:** `ffmpeg`/`ffprobe` are absent from this sandbox, so every `requires_ffmpeg` test — including this one and the 13.2 gate — skips under the plain test command. A static ffmpeg 7.0.2 build was fetched to `/projects/sandbox/.ffbin` (outside the repo, never added to the repo or to the default `PATH`) purely to *execute* these tests during verification; all 52 tests in `test_pipeline_effects.py`, `test_engine_host.py`, `test_effects_compositor.py` and `test_pipeline_degradation.py` pass with it on `PATH`, with nothing skipped.
    - _Requirements: 15.8, 23.6_

  - [x] 15.5 Re-run the all-off parity gate and confirm it still passes
    ...
    - **Completed 15.5 — the gate holds, unmodified; the seam did not leak into the render path.** `tests/test_pipeline_degradation.py` has **zero** diff (`git diff --stat` empty), and all 17 tests in it pass, including `test_p34_all_engines_off_reproduces_v080_exactly` (13.1) and `test_all_off_ffmpeg_parity_matches_unhooked_baseline` (13.2). Both were run with real ffmpeg on `PATH`, so 13.2 **actually executed** rather than skipping as it does in the bare sandbox — a stronger confirmation than the task strictly required: same clip count, same `effects_applied`, same ffmpeg invocation count, `render_clip` still returning `None` for an all-off clip, and no `engines/` directory beneath the job `temp_dir`. That holds for the expected structural reason: the new mapping is built inside the `if host.active:` COMPOSE block, so an all-off run never even constructs it, and Clip_Metadata is never read by the compositor or forwarded into any ffmpeg argv.
