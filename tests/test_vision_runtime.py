...
def _face_frame(cv2, width: int = 1920, height: int = 1080):
    ...
    cv2.ellipse(
        canvas, (cx, cy + int(0.06 * face_h)),
        (max(2, int(0.05 * face_w)), int(0.10 * face_h)), 0, 0, 360, (140, 158, 178), -1,
    )
    cv2.ellipse(
        canvas, (cx, cy + int(0.24 * face_h)),
        (int(0.17 * face_w), max(2, int(0.045 * face_h))), 0, 0, 360, (70, 70, 95), -1,
    )
    cv2.ellipse(
        canvas, (cx, cy - int(0.42 * face_h)),
        (face_w // 2, int(0.16 * face_h)), 0, 0, 360, (45, 40, 40), -1,
    )
