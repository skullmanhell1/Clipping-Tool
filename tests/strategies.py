...
def st_font_availability(
    ...
    *,
    ...
    availability.update(
        {f"font:{family}": bool(flag) for family, flag in zip(ladder, flags, strict=True)}
    )

    available_families = tuple(family for family, flag in zip(ladder, flags, strict=True) if flag)
