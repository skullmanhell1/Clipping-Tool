...
def test_pipeline_applies_effects(make_video, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda *a, **k: [
            ClipCandidate(
                start=0.0, end=6.0, score=88.0, reason="t", title="T", text="This is fire and money"
            )
        ],
    )
...
def _stub_source_transcript(monkeypatch, pl, span):
    ...
    monkeypatch.setattr(
        pl.sel,
        "select_moments",
        lambda *a, **k: [ClipCandidate(start=start, end=end, score=77.0, text="hello there")],
    )
...
def _inject_isolated_host(monkeypatch, pl, engines):
    ...
    def factory(options, **kwargs):
        return Engine_Host(
            options, registry=registry, capabilities=report, storage=storage, clock=clock, **kwargs
        )
...
def test_audio_stage_engine_may_replace_clip_media(make_video, tmp_path, monkeypatch):
    ...
    replacement = make_video("engine_audio_replacement.mp4", duration=clip_length, w=320, h=240)
    _stub_source_transcript(monkeypatch, pl, span)

    # (A) the engine replaces the clip media.
    engine = FakeEngine("audio_probe", Engine_Stage.AUDIO, media=replacement, markers=("replaced",))
    _inject_isolated_host(monkeypatch, pl, [engine])
    ...
    opts = Engine_Processing_Options(
        ...
        metadata=False,
        aspect="9:16",
        audio_probe_enabled=True,
    )
    clips = pl.run_pipeline(src, opts, clips_dir=tmp_path / "clips_a", temp_dir=tmp_path / "tmp_a")
    ...
    raiser = RaisingEngine(
        "audio_raiser", Engine_Stage.AUDIO, exc=RuntimeError("audio engine exploded")
    )
    ...
    opts_failing = Engine_Processing_Options(
        ...
        metadata=False,
        aspect="9:16",
        audio_raiser_enabled=True,
    )
    failed_clips = pl.run_pipeline(
        src, opts_failing, clips_dir=tmp_path / "clips_b", temp_dir=tmp_path / "tmp_b"
    )
...
def test_pipeline_supplies_clip_metadata_to_compose_stage_engines(
    ...
    clips = pl.run_pipeline(
        src, opts, clips_dir=tmp_path / "clips", temp_dir=tmp_path / "tmp", llm_client=llm
    )
