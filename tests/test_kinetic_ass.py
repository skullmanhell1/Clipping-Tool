...
def test_p7_visible_text_preserves_every_word_in_order(timeline, style, reveal, option_data):
    ...
    for source_words, source_duration, exact in (
        ...
        emitted_words = []
        for cue, text in zip(plan.cues, texts, strict=True):
            ...
            for group, rendered in zip(groups, rendered_lines, strict=True):
...
def test_p8_shared_styles_reproduce_build_word_span_semantics(timeline, style, option_data):
    ...
    for source_words, source_duration in (
        ...
        for cue, text in zip(plan.cues, _default_texts(emit_ass(plan)), strict=True):
